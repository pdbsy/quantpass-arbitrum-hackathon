...
function bytes(input: HexData): Uint8Array {
  const normalized = asHexData(input);
  const result = new Uint8Array((normalized.length - 2) / 2);
  for (let index = 0; index < result.length; index++)
    result[index] = Number.parseInt(normalized.slice(2 + index * 2, 4 + index * 2), 16);
  return result;
}

export function keccak256Evm(input: HexData): BlockHash {
  const inputBytes = bytes(input);
  const paddedLength = Math.ceil((inputBytes.length + 1) / rateBytes) * rateBytes;
  ...
  return asBlockHash(`0x${[...output].map((value) => value.toString(16).padStart(2, '0')).join('')}`);
}
