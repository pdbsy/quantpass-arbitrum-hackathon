...
>
> 范围：仅限测试网、模拟资金和可审计工程验证；未通过全部门禁前禁止主网、真实资金以及无人值守或自主交易。
...
- [ ] **SUPPLY-001 · 强化仓库与供应链策略** — 进行中 / P1 / High / M
  ...
  - 证据：`.github/CODEOWNERS`、`.github/dependabot.yml`、`.github/workflows/codeql.yml`、`.github/workflows/dependency-review.yml`、`SECURITY.md`、`docs/security/SUPPLY-CHAIN.md`、`docs/security/github-security-settings.json`、`docs/security/npm-sbom.spdx.json`、`planning/supply-chain-policy.json`、`test/supply-chain.test.mjs`、`tools/check-supply-chain.mjs`
...
- [ ] **SPEC-001 · 链上会计、资产与舍入规格** — 待排期 / P0 / Critical / L
  ...
  - 验收：
    ...
    - 拒绝 fee-on-transfer、rebasing、回调、可变实现和非标准返回 token
...
- [ ] **SPEC-002 · 权限、事件、错误与重放规格** — 待排期 / P0 / Critical / M
  ...
  - 交付：函数权限矩阵；事件/错误目录；nonce/deadline 规则；不可逆暂停、Owner 退出与新地址迁移状态机
  - 验收：
    - 所有状态写入都有明确角色和自定义错误
    - 事件足以独立重建关键状态且不记录秘密
    - 跨链、跨合约、跨账户和过期签名不能重放
...
- [ ] **CON-001 · 实现最小非托管 Vault 合约** — 待排期 / P0 / Critical / L
  - 目标：按冻结规格实现最小资产托管与逐笔双签执行边界；只允许类型化固定 target/selector，不包含任意外部调用。
  - 依赖：`ABI-001`、`TOOL-001`
  ...
  - 验收：
    ...
    - 所有外部写函数具备授权、暂停与重入分析
    - 不包含任意 target/calldata、delegatecall 或可注入实现地址；类型化固定调用逐项验证
...
- [ ] **TST-002 · Fuzz、Invariant 与模型差分验证** — 待排期 / P0 / Critical / L
  ...
  - 验收：
    ...
    - 不可逆暂停、角色失效、显式迁移、失败 token 与随机顺序均纳入生成
    - 失败可用种子和命令序列稳定复现
