...
**Goal:** Close the remaining engineering coverage gap using meaningful behavioral tests, then produce one final acceptance and dashboard closure.
...
- [ ] P0: malformed restored SQLite state, canonical event replay/conflicts and client snapshot owner/revision/account relationships. Append assertions to test/chain-store.test.ts and test/ui-product-adapter.test.ts. Assert rejection plus unchanged stored/published state; test legal controls too.
