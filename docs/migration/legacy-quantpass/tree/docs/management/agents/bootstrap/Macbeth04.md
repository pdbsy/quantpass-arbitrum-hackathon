...
For initial check-in, use `macbeth04/af-agent-setup` and your own Draft PR `[Macbeth04][AF-AGENT-SETUP] Initialize worker environment`. Each commit subject contains `[Macbeth04]`; its body contains `Agent-ID: Macbeth04` and matching `Task-ID`. Publish your own CHECK_IN from this session.
