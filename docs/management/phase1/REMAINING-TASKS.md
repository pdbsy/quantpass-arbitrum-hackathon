...
| --- | --- | --- | --- | --- | --- |
| PH1-01 | 01 | 固定基线、登记、文档/路线图/状态板同步 | IN_PROGRESS | 所有来源映射；真实 ACK；无旧状态冒充当前 | 01维护准确来源、逐条分支缺口、审阅和C/R/S。当前1069初始缺口；后续两批各11个相对6a完整报告的同源同图真实新命中、不互相重复。最终同一候选重新全量测量，已允许正常公开推送。 / FINAL_CANDIDATE_FREEZE |
...
| PH1-08 | 04 | 暖色英文实际入口、六页信息和数据来源标签 | SCOPED_LOCAL_VERIFIED_FINAL_CANDIDATE_PENDING | 主页/市场/详情/账户/用户论坛/排名准确；不改未使用React入口 | 7e683e7产品修复经05独立旧界面17检查和M3 26检查，零page/CSP/blocked错误。蓝色管理看板新增缺失嵌套状态拒绝及传输错误边界，最终浏览器验收待准确候选。 / FINAL_CANDIDATE_ACCEPTANCE |
| PH1-09 | 04 | 发行/分配或获取 Pass、自由转账、创建/选择 Vault 操作入口 | SCOPED_LOCAL_VERIFIED_FINAL_CANDIDATE_PENDING | 1 PASS容量不等于价格；转账18位；创建或选择 Vault；受审 allowlist 选择满足产品路径，创建保留显式 Owner 部署流程；切换废弃旧意图/模拟 | 固定供应首发分配、18位自由转账、allowlist Vault选择范围不变。04已修复review/confirm/API晚到结果影响新弹窗问题；后续真实入口集中补测和逐条限定证明继续。 / FINAL_CANDIDATE_ACCEPTANCE |
...
| PH1-12 | 05 | 功能验收矩阵、准确覆盖率与跨层独立复测 | IN_PROGRESS_NOT_COMPLETE | 测量总体90%目标和关键授权会计100%分支要求；不足仍FAIL/BLOCKED，不拿数量替代 | 当前基线f966dd2未命中1069。6a完整13工作流行12432/12821、语句15000/15562、函数2263/2372、分支10418/10888仅为中间值；继续验证合理可达路径，最终计数/平台/限定证明/未命中语义分列并独立复核。 / FINAL_CANDIDATE_COVERAGE_AND_METHOD_REVIEW |
| PH1-13 | 05 | 独立最终安全复核 | BLOCKED_EXTERNAL_REVIEW | 服务限制未解除不得重试规避；普通功能复测不能代替；无伪造通过 | 05 2026-09-19服务失败；新05状态报告 / EXTERNAL_REVIEW |
| PH1-14 | 06 | 7必需CI、附加任务、合约/scanner覆盖和证据真实性 | FINAL_CANDIDATE_HOSTED_PENDING | 9jobs每关键步骤实际执行；错误失败关闭；旧CI失败不改写 | Public仓库及正常发布/现有标准CI已授权；7必需+2现有附加job及承诺CodeQL/Dependency Review必须对应最终最新head。远程3a78e34旧9绿不作新候选证明；Windows超时用例1142996仅静态审阅和macOS SKIP，待真实Windows。 / EXACT_HEAD_REMOTE_CI |
...
| PH1-19 | 01 | 统一候选、干净环境复现、演示/恢复和交付包 | IN_PROGRESS | 实现/本地/CI/QA/review/merge/testnet各状态分列；所有限制留存 | 01集成当前修复、身份和169项迁移来源摘要核验。所有局部审阅完成后统一C/R/S、14工作流、完整剩余清单、公开正常发布及准确head托管验证；不再将中间值当最终交付。 / FINAL_CANDIDATE_FREEZE_AND_ACCEPTANCE |
| PH1-20 | 01 | 核验实际GitHub审批和合并条件；本轮到Ready后停止 | REVIEW_BLOCKED_STOP_AT_READY | 全部准确head门禁和实际审批条件满足；不降低规则、不代签、不自动merge | ruleset22507334实际要求1名独立审批、CODEOWNERS、末次push审批、解决讨论及7 required checks。pdbsy不能自批，worker审阅不是GitHub审批；本轮只到Ready，不合并。 / GITHUB_INDEPENDENT_REVIEW |
