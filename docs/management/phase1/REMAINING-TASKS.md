...
本轮起始候选 `3a78e34ba933f1e3239424d3bf0b1e27b1a65bb8`；本地主分支基线 `18f5352070910a867b9729b031aa2e3951785e01`。托管执行与状态查询均已由用户暂停，详见 [暂停边界](CI-BUDGET-PAUSE.md)。现有 711/711、C/R/S 和 CI 记录仅绑定其历史准确候选。本轮本地修改必须重新验收；整体覆盖率仍 NOT_MEASURED。GITLEAKS-FP-001 已实现，不再等待用户批准。

所有字段以 [JSON 矩阵](remaining-tasks.json) 为同一事实源；65%finish 不是覆盖率或当前完成率。

| Task | Owner | 目标 | 当前状态 | 关闭条件 | 证据 / 阻塞 |
| --- | --- | --- | --- | --- | --- |
| PH1-01 | 01 | 固定基线、登记、文档/路线图/状态板同步 | IN_PROGRESS | 所有来源映射；真实 ACK；无旧状态冒充当前 | ASSIGNMENTS.md；PR21；本轮 PR / NONE |
...
| PH1-12 | 05 | 功能验收矩阵、准确覆盖率与跨层独立复测 | IN_PROGRESS_NOT_COMPLETE | 测量总体90%目标和关键授权会计100%分支要求；不足仍FAIL/BLOCKED，不拿数量替代 | 05对3a78e34的711/711、C/R/S、身份与环境已有独立通过；总体正式覆盖仍NOT_MEASURED。历史探索诊断不用于新候选；本轮方法准入/真实路径/竞态/独立复验继续 / BROWSER_MEASUREMENT_AND_CANDIDATE_DEPENDENCY |
...
| PH1-18 | 01 | 4项合约NOT_RUN采集边界 | ASSESS_AND_COMPLETE_IF_REQUIRED | 不得手填PASS；若阶段硬门槛则真实解决；否则明确两套证据覆盖 | tools/management-dashboard/checks.mjs；contract job / SCOPE_EVIDENCE |
...
| PH1-23 | 04 | 确认中Vault A→B与A→B→A失效；Vault action/Pass transfer/deposit approval | CONFIRM_INFLIGHT_VAULT_SELECTION_RACE | 05定向独立复测；真正钱包发送前失效，已发送操作按原身份跟踪 | 本轮待实际交付，不预写PASS / ENGINEERING_VALIDATION |
| PH1-24 | 06 | 九job本地映射、Woodpecker/Jenkins/act比较、PoC与接回方案 | LOCAL_CI_ALTERNATIVE_IN_PROGRESS | 真实失败/超时/日志缺失负例；无新服务/凭据；未运行平台NOT_RUN | 本轮待实际交付，不预写PASS / LOCAL_VALIDATION |
| PH1-25 | 05 | 使用保存日志调查Windows文件级进程失败 | WINDOWS_ROOT_CAUSE_UNDETERMINED | 06协作；无合格原生Windows则NOT_RUN，不用macOS证明Windows通过 | 本轮待实际交付，不预写PASS / PLATFORM_AND_EVIDENCE |
