...
Historical setup example (not the current task): use `macbeth02/af-agent-setup` and your own Draft PR `[Macbeth02][AF-AGENT-SETUP] Initialize worker environment`. Each commit subject contains `[Macbeth02]`; its body contains `Agent-ID: Macbeth02` and matching `Task-ID`. Publish your own CHECK_IN and, after reading Macbeth01's real NOTICE, your own ACK.
