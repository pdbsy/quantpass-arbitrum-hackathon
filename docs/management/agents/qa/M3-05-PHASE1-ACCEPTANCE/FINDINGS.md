...
| M3-05-P1-RACE-01  | `AWAITING_FIX / NOT_RUN`                | The newly required delayed wallet-operation/Vault-selection race fix has not arrived from Macbeth04. Prior browser evidence covers review invalidation and isolation but is not this new delayed-operation case. | Exact 04 fix SHA, focused regression, then Macbeth05 independent local retest on the integrated source.                                                                |
