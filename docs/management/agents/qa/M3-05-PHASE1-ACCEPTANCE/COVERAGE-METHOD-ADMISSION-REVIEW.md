...
3. **Complete lifecycle declaration.** Before execution, enumerate every expected Node process, worker, browser page/context and coverage interval. Replaced environments must receive the approved bootstrap explicitly. Missing, duplicate, late, abruptly terminated or `SIGKILL` lifecycles must remain visible and contribute zero; a successful outer command cannot make an undeclared missing child disappear.
4. **Browser capture completeness.** Prove coverage capture for every product page/context and navigation used by the accepted browser workflow, including close and error paths. The current browser shim creates observations only when a wrapped capture sees counters and has no independent expected-page manifest.
...
The correct current status remains `OVERALL_COVERAGE_NOT_MEASURED`; `M3-05-P1-001` stays open. The next engineering work is to version the collector/report entry, close lifecycle and browser completeness gaps, decide and test the forum alias policy, remediate the reported source gaps, and then perform one independent exact-candidate replay.
