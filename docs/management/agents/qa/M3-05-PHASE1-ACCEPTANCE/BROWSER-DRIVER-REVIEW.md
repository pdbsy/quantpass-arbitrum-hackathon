...
- worker 只读源：`/Users/ikol/.codex/worktrees/17d0/AlphaForge`，接收时干净。
...
首次浏览器执行因沙箱拒绝 `127.0.0.1:4287` 监听报 EPERM，原失败日志保留。按已授权 local/mock 范围允许本地监听后重跑成功，没有改变源码或断言。所有原始日志、结果 JSON、截图与工具复制回执见 [证据清单](evidence/browser-driver-review/SHA256SUMS.json)。
