...
- Workflow/job profiles impose explicit permission ceilings: CI (Linux, Windows and macOS) and dependency review allow only `contents: read`; only CodeQL's `analyze` job may also read packages and write security events. New workflows, jobs, events and permission scopes require a reviewed profile change.
...
- The active `master-protection` ruleset (ID `22507334`) has no bypass actor, requires pull requests, strict `verify`/CodeQL/dependency-review checks, linear history and resolved conversations, and blocks deletion and force pushes. On 2026-09-13 the actual rules were strengthened to require verify-windows and verify-macos alongside the existing checks, one approval, CODEOWNER and last-push approval. `pdbsy` is still the only collaborator, so these rules do not provide independent approval. See [PR 11 before/after readback](pr11-master-protection.json).
