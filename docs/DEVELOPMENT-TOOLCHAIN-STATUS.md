...
下载只从 Node 官方发行地址及现有供应链策略指定的 npm registry 获取精确资产；校验 SHA-256 / SHA-512 后才执行。首次 Node 下载截断且摘要不符，被拒绝，未执行。随后有界分段下载的完整 52,909,993 字节归档通过官方 SHA-256：`bed7eea5325e1108f32ce5228ddd6a5f0f08a499ee42aa7442aea583702f6057`。npm 包通过 registry SHA-512 校验；实际执行版本为 Node 24.21.0 / npm 11.19.1。`npm ci --ignore-scripts` 成功，安装 193 个本平台包，217 个锁定包的图保持不变。完整本地回归 240/240 通过；环境准入的正式 fnm 条件仍为 BLOCKED，不能把临时工具链工程测试等同机器准入。
